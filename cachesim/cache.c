...
uint32_t cache_read(uintptr_t addr) {
  ...
  for (int i = 0; i < 4; i++){
    if (Cache[group_num][i].tag == tag && Cache[group_num][i].valid_bit == true){
      ret = (void *)(Cache[group_num][i].Block) + block_num;
      is_hit = true;
      break;
    }
  }
  ...
  if (is_hit == false){
    ...
    if (empty == false){
      ...
      ret = (void *)(Cache[group_num][chose_place].Block) + block_num;
    }
  }
  return *ret;
}
...
void cache_write(uintptr_t addr, uint32_t data, uint32_t wmask) {
  ...
  for (int i = 0; i < 4; i++){
    if (Cache[group_num][i].tag == tag && Cache[group_num][i].valid_bit == true){
      is_hit = true;
      uint32_t *place = (void *)(Cache[group_num][i].Block) + block_num;
  ...
  if (is_hit == false){
    ...
    if (empty == false){
      ...
      mem_read(addr >> 6, Cache[group_num][chose_place].Block);
      uint32_t *place = (void *)(Cache[group_num][chose_place].Block) + block_num;
      *place = (*place & ~wmask) | (data & wmask);
      Cache[group_num][chose_place].tag = tag;
      Cache[group_num][chose_place].valid_bit = true;
      Cache[group_num][chose_place].dirty_bit = false;
